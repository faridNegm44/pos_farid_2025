    ...
    public function store(request $request)
    {
        if (request()->ajax()){
            ...
            $valid = $this->validate($request, [
                ...
                'client' => 'required_without:supplier|nullable|integer|exists:clients_and_suppliers,id',
                'supplier' => 'required_without:client|nullable|integer|exists:clients_and_suppliers,id',
            ...
            DB::transaction(function(){
                ...
                $getId = DB::table('treasury_bill_dets')->insertGetId([
                    ...
                    'bill_id' => 0,
                    'bill_type' => 0, 
                    'client_supplier_id' => $lastRecordClientSupplier->client_supplier_id, 
    ...
    public function datatable()
    {
        ...
        return DataTables::of($all)
            ...
            ->addColumn('treasury_money_after', function($res){
                return $res->treasury_money_after;
            })
            ->addColumn('remaining_money', function($res){
                if($res->remaining_money < 0){
                    return '<strong class="bg rounded bg-secondary text-white" style="font-size: 13px;padding: 0 4px;">'.$res->remaining_money.'</strong>';
                }else{
                    return '<strong style="font-size: 13px;padding: 0 4px">'.$res->remaining_money.'</strong>';
                }
            })
            ->addColumn('amount_money', function($res){
                if($res->treasury_type === 'اذن توريد نقدية'){
                    return '<strong class="bg rounded bg-success text-white" style="font-size: 13px;padding: 0 4px">'.$res->amount_money.'</strong>';
                    
                }elseif($res->treasury_type === 'اذن صرف نقدية'){
                    return '<strong class="bg rounded bg-danger text-white" style="font-size: 13px;padding: 0 4px">'.$res->amount_money.'</strong>';

                }elseif($res->treasury_type === 'اذن ارتجاع نقدية'){
                    return '<strong class="bg rounded bg-dark text-white" style="font-size: 13px;padding: 0 4px">'.$res->amount_money.'</strong>';
                }
            })            
