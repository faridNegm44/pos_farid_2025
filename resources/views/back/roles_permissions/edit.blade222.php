...
<div class="container-fluid">
    ...
    <div class="card" style="padding: 10px" id="permisions">
        ...
        $models = [
        // start first add
        'financialYears', 'stores', 'financial_treasury', 'units', 'companies', 'productsCategories',
        'products_sub_category', 'products', 'taswea_products', 'transfer_between_stores', 'clients', 'clients_report',
        'clients_account_statement', 'suppliers', 'suppliers_report', 'suppliers_account_statement',
        'taswea_client_supplier', 'partners', 'partners_report', 'partners_account_statement', 'taswea_partners',
        'sales', 'sales_create', 'sales_return', 'products_stock_alert', 'purchases', 'purchases_create',
        'purchases_return', 'treasury_bills', 'treasury_bills_create', 'treasury_bills_report',
        'transfer_between_storages', 'expenses', 'expenses_report', 'users', 'settings', 'roles_permissions'
        // end first add
        ];
        $count = 1;
        @endphp
        ...
        <form action="{{ url('roles_permissions/update/'.$find['id']) }}" method="POST">
            ...
            <div>
                    ...
                    <table class="table table-bordered table-striped table-hover nowrap">
                        ...
                        <tbody>
                            ...
                            <tr style="" id="{{ $model }}">
                                ...
                                <input class="form-check-input" type="checkbox" name="{{ $model }}_create"
                                    {{ old(''.$model.'_create') ? 'checked' : '' }}
                                    {{ $find[''.$model.'_create'] == 1 ? 'checked' : '' }} id="{{ $model }}_create" />
                ...
                <div class="form-check me-3 me-lg-5" id="{{ $model }}_view_div">
                    <input class="form-check-input" type="checkbox" name="{{ $model }}_view"
                        {{ old(''.$model.'_view') ? 'checked' : '' }}
                        {{ $find[''.$model.'_view'] == 1 ? 'checked' : '' }} id="{{ $model }}_view" />
                ...
                <div class="form-check me-3 me-lg-5" id="{{ $model }}_update_div">
                    <input class="form-check-input" type="checkbox" name="{{ $model }}_update"
                        {{ old(''.$model.'_update') ? 'checked' : '' }}
                        {{ $find[''.$model.'_update'] == 1 ? 'checked' : '' }} id="{{ $model }}_update" />
                ...
                <div class="form-check me-3 me-lg-5" id="{{ $model }}_delete_div">
                    <input class="form-check-input" type="checkbox" name="{{ $model }}_delete"
                        {{ old(''.$model.'_delete') ? 'checked' : '' }}
                        {{ $find[''.$model.'_delete'] == 1 ? 'checked' : '' }} id="{{ $model }}_delete" />
    ...
    <div class="d-flex flex-wrap gap-2">
        <button type="submit" class="btn btn-primary-gradient waves-effect waves-light" id="save"
            style="display: block;width: 20%;height: 50px;margin: 0 auto;">تحديث</button>
    </div>
</div>
