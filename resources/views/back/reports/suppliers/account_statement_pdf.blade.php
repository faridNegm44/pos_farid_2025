...
<head>
    ...
    <title>{{ $pageNameAr }} - {{ $results[0]->supplierName }} - {{ date('d-m-Y H-i-s') }}</title>
    ...
    <div class="container" style="border: 1px solid #000;">
        <div class="">
            ...
            <table width="100%" border="1" cellpadding="10" cellspacing="0">
                ...
                <tbody>
                    <tr>
                        <td>{{ $results[0]->supplierName }}</td>
                        <td>{{ $results[0]->supplierPhone }}</td>
                        <td>{{ $results[0]->supplierAddress }}</td>
                    </tr>
                </tbody>
            </table>
        ...
        <div>
            @foreach ($results as $bill)
                ...
                {{----------------------------- start $bill->treasury_type === 'اذن توريد نقدية' && $bill->bill_type === 'اضافة فاتورة مشتريات'  ------------------------------------}}
                {{-----------------------------------------------------------------------------------------------}}          
                @elseif ($bill->treasury_type == 'اذن توريد نقدية' && $bill->bill_type == 'اضافة فاتورة مشتريات')
                    ...
                    {{-------------------------------- end $bill->treasury_type === 'اذن توريد نقدية' && $bill->bill_type === 'اضافة فاتورة مشتريات'  ------------------------------------}}
                    {{-----------------------------------------------------------------------------------------------}}  
                ...
                <p style="margin: 0 auto;text-align: center;border: 1px dotted #000;width: 35%;">
                    <strong>حالة المورد بعد الفاتورة:</strong> 
            ...
            <h3 style="font-weight: bold;padding-top: 15px;">
                @foreach ($results as $bill)
                    @if ($loop->last)
                        ...
                        @elseif($bill->remaining_money > 0)
                            الرصيد الحالي للمورد (علية):
                            {{ display_number( $bill->remaining_money ) }}

                        @else
                            الرصيد الحالي للمورد (لة):
                            {{ display_number( $bill->remaining_money ) }}
                        @endif
                    @endif
                @endforeach
